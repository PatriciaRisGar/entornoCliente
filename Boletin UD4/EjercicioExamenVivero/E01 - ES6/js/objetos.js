...
class Vivero {
  ...
  altaArbol(arbol) {
    ...
    let encontrado =
      this.arboles.filter((elem) => elem.codigo == arbol.codigo).length == 1;
    //let encontrado = this.arboles.includes(arbol);
    //let encontrado = this.arboles.find((elem) => elem.codigo == arbol.codigo) != undefined;
    if (!encontrado) {
  ...
  buscarArbol(codigo) {
    //Se puede reescribir como findIndex
    let i = 0,
      encontrado = false;
    while (i < this.arboles.length && !encontrado) {
  ...
  tallajeArbol(codigo, tallaje) {
    ...
    let posicion = this.arboles.findIndex((elem) => elem.codigo == codigo);
    if (posicion < 0) {
      ...
      mensajeSalida +=
        this.arboles[posicion] instanceof Perenne ? "Perenne" : "Caduco";
    }
